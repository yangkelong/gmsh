...
// 如果我们想在全局范围内改变这些网格尺寸（而不改变上面的定义），我们可以使用 -clscale 选项在命令行上
// 为所有网格尺寸提供全局缩放因子（或者在选项文件中使用 Mesh.MeshSizeFactor）。例如，使用：
// gmsh t5.geo -clscale 1
// 这个输入文件产生了大约 3000 个节点和 14,000 个四面体的网格。使用
// gmsh t5.geo -clscale 0.2
// 网格大约有 231,000 个节点和 1,360,000 个四面体。您可以在图形用户界面中使用 工具->统计 菜单检查网格统计信息。
//
// 有关网格尺寸的更多信息，请参见 t10.geo。
// 继续定义一些基本实体来描述一个截断立方体：
...
// 现在使用用户定义的宏 而不是使用包含文件 来在立方体中雕刻一些孔洞：
Macro CheeseHole
  ...
  // rest.
// 在以下命令中，我们使用保留的变量名 newp，它自动选择一个新的点标签。类似于 newp，
// 特殊变量 newc，newcl，news，newsl 和 newv 分别选择新的曲线、曲线环、表面、表面环和体积标签。
//
// 如果 Geometry.OldNewReg 设置为 0，则新标签被选择为每个类别（点、曲线、曲线环等）的最高当前标签加一。
// 默认情况下，为了向后兼容，Geometry.OldNewReg 设置为 1，并且只使用两个类别：一个用于点，另一个用于其余部分。
